...
const TextPressure: React.FC<TextPressureProps> = ({
  ...
  // Memoize chars array to prevent unnecessary re-renders
  ...
  const getAttr = useCallback(
    (distance: number, minVal: number, maxVal: number, maxDist: number) => {
  ...
  useEffect(() => {
    setSize();
    const debouncedResize = debounce(setSize, 150);
  ...
  }, [width, weight, italic, alpha, dist, getAttr]);
  ...
  return (
    ...
    >
      ...
      <h1
        ref={titleRef}
        className={`text-pressure-title tracking-widest ${className} ${
